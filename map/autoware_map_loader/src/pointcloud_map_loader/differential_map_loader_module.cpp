...
{
  get_differential_pcd_maps_service_ = AUTOWARE_CREATE_SERVICE2_ON_NODE(
    GetDifferentialPointCloudMap, node, "service/get_differential_pcd_map",
    [this](
      const AUTOWARE_SERVICE_REQUEST_PTR(GetDifferentialPointCloudMap) & request,
      const AUTOWARE_SERVICE_RESPONSE_PTR(GetDifferentialPointCloudMap) & response) {
      this->on_service_get_differential_point_cloud_map(request, response);
    });
}
...
bool DifferentialMapLoaderModule::on_service_get_differential_point_cloud_map(
  const AUTOWARE_SERVICE_REQUEST_PTR(GetDifferentialPointCloudMap) & req,
  const AUTOWARE_SERVICE_RESPONSE_PTR(GetDifferentialPointCloudMap) & res) const
{
  auto area = req->area;
